...
router.get('/linked-clients', auth_middleware_1.authMiddleware, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    var _a, _b, _c;
    try {
        ...
        const affiliateProfile = yield prisma.affiliate_profiles.findUnique({
            where: { userId: userId }
        });
        if (!affiliateProfile) {
        ...
        const now = new Date();
        const adminLinks = yield prisma.affiliate_client_links.findMany({
            where: {
        ...
        const clientsWithOrders = yield prisma.orders.findMany({
            where: {
        ...
        for (const link of adminLinks) {
            ...
            const ordersCount = yield prisma.orders.count({
                where: {
                    userId: client.id,
            ...
            const commissionsSum = yield prisma.commission_transactions.aggregate({
                where: {
                    affiliate_id: affiliateProfile.id,
                    orders: {
                        userId: client.id,
            ...
            const totalCommissions = Number(((_b = commissionsSum._sum) === null || _b === void 0 ? void 0 : _b.amount) || 0);
        ...
        for (const orderData of clientsWithOrders) {
            ...
            const totalCommissions = Number(((_c = commissionsSum._sum) === null || _c === void 0 ? void 0 : _c.amount) || 0);
            allLinkedClients.push({
                client: {
                    id: client.id,
                    firstName: client.first_name,

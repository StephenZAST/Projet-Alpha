...
class AppController extends GetxController {
  ...
  Duration get currentSessionDuration =>
      DateTime.now().difference(_sessionStartTime.value);
  ...
  Future<void> _loadSettings() async {
    try {
      final settings =
          _storage.read<Map<String, dynamic>>(StorageKeys.appSettings) ?? {};
      ...
      _locationTrackingEnabled.value =
          settings['location_tracking_enabled'] as bool? ?? true;
      _autoRefreshEnabled.value =
          settings['auto_refresh_enabled'] as bool? ?? true;
  ...
  void _endSession() {
    ...
    debugPrint(
        '🏁 Session terminée. Durée: ${_formatDuration(sessionDuration)}');
  }
  ...
  // ==========================================================================
  // 🔐 VÉRIFICATION AUTH AU DÉMARRAGE / HOT RELOAD
  // ==========================================================================

  /// Vérifie l'état d'authentification et redirige vers la bonne page.
  /// - Si authentifié: on force la navigation vers le dashboard (préféré après hot reload)
  /// - Sinon: on redirige vers la page de connexion
  Future<void> _checkAuthenticationAndRedirect() async {
    try {
      // Repose sur l'état actuel du service d'authentification
      if (_authService.isAuthenticated) {
        // S'assurer que l'app revient sur le dashboard après hot reload
